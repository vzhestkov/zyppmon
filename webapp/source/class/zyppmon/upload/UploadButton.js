 ...
 *
 */
qx.Class.define("zyppmon.upload.UploadButton",
{
      ...
      var control;
		...
		control.addListener("change", function(e) {
			...
			if ( controlDom.files && controlDom.files.length > 0 ) {
				for ( var i = 0; i < controlDom.files.length; i++ ) {
					namesList += i == 0 ? controlDom.files[i].name : "; " + controlDom.files[i].name;
					totalSize += typeof controlDom.files[i].fileSize != "undefined" ? controlDom.files[i].fileSize : controlDom.files[i].size;
				}
			}
			this.setFileSize(totalSize);
			this.setFileName(namesList);
			this.fireDataEvent('changeFileName', namesList);
		},this);
